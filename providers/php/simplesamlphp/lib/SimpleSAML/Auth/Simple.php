...
class SimpleSAML_Auth_Simple {
	...
	*/
	...
	{
		$idp_info = $this->getAuthSource();
		$attrs = $this->getAttributes();

		$auth_url = $idp_info->getAuthEndpoint();
        $url_parsed = parse_url($auth_url);
        $ISS = $url_parsed['scheme']."://".$url_parsed['host'].":".$url_parsed['port'];
        //$ISS = $_SERVER['HTTP_X_USER_ISS'];

		$sub = $attrs['uid'][0];
		$domain = $_SERVER['SERVER_ADDR'].":".$_SERVER['SERVER_PORT'];
		$uri = $_SERVER['REQUEST_URI'];
		$scheme = $_SERVER['REQUEST_SCHEME'];
		$method = $_SERVER['REQUEST_METHOD'];
		$from = $_SERVER['HTTP_X_DATA_FROM'];
		$to = $idp_info->getClientId();

		$handle = curl_init($ISS."/Validate/ValidatePermissionOnUrl");
		curl_setopt($handle, CURLOPT_POSTFIELDS, http_build_query(array('sub' => $sub,
										'uri' => $uri,
										'domain' => $domain,
										'scheme' => $scheme,
										'method' => $method,
